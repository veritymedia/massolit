export interface ExamRaw {
  ...
  duration: string // format HH:mm or mm (minutes) or 2025-05-06T08:00:00
...
export function assignTeachersToExams(
  ...
  function teachesSubject(teacherSubjects: string[], examSubject: string): boolean {
  ...
  function isTeacherAvailable(
    ...
    return lessons.every(
      (lesson) =>
        !teacher.schedule.some((slot) => slot.day === dayOfWeek && slot.period === lesson),
    )
  }
  ...
  function calculateLessonUtilization(
    ...
    for (const slot of teacher.schedule) {
      ...
      const lessonStart = timeToMinutes(config.schoolTimetable[slot.period].start)
  ...
  // Process exams data
  const exams: Exam[] = examsRaw.map((exam, id) => {
    const startMinutes = timeToMinutes(exam.start)
    const durationMinutes = parseDuration(exam.duration)

    // .date exists.
    let dayOfWeek: number = NaN
    if (typeof exam.date === 'string' && exam.date.includes('/')) {
      dayOfWeek = getDayOfWeekFromSlashDate(exam.date)
    } else if (exam.date && exam.date.includes('T')) {
      dayOfWeek = new Date(exam.date).getDay()
    }

    let dateFromStart = ''
    if (exam.start.includes('T')) {
      dayOfWeek = new Date(exam.start).getDay()
      dateFromStart = exam.start.split('T')[0]
    }
    ...
    return {
      ...
      dayOfWeek: dayOfWeek,
      proctors: [],
      date: dateFromStart ? dateFromStart : exam.date,
    }
  })
    ...
    for (const [room, roomExams] of roomsMap.entries()) {
      ...
      for (let i = 0; i < sortedTimePoints.length - 1; i++) {
        ...
        const activeExams = roomExams.filter(
          (exam) => exam.startMinutes <= startTime && exam.endMinutes >= endTime,
        )

        if (activeExams.length === 0) continue

        const dayOfWeek = getDayOfWeekFromSlashDate(date)
        ...
        if (startTime < firstLessonStart) {
          ...
          const examSubjects = new Set(activeExams.map((e) => e.subject.toLowerCase()))

          // Find eligible teachers for this time slice
          const eligibleTeachers = teachers.filter((teacher) => {
            // Check availability
            const available = isTeacherAvailable(teacher, dayOfWeek, remainingSliceStart, endTime)
          ...
          const teachersWithScore = eligibleTeachers.map((teacher) => ({
            teacher,
            score:
              calculateLessonUtilization(remainingSliceStart, endTime, dayOfWeek, teacher) -
          ...
          const neededTeachers = Math.min(parallel_teachers, activeExams.length)
          const selectedTeachers = teachersWithScore.slice(0, neededTeachers).map((t) => t.teacher)
          ...
          const eligibleTeachers = teachers.filter((teacher) => {
            // Check availability
            const available = isTeacherAvailable(teacher, dayOfWeek, startTime, endTime)
            if (!available) return false
          ...
          const teachersWithScore = eligibleTeachers.map((teacher) => ({
            teacher,
            score:
              calculateLessonUtilization(startTime, endTime, dayOfWeek, teacher) -
              teacher.assignedMinutes / 1000, // Balance factor
          }))
  ...
  return exams.map(
    ({ id, startMinutes, endMinutes, dayOfWeek, ...rest }) => rest,
  ) as ExamWithProctors[]
}
